...
if(!userId){
  return res.send({msg: "Please enter UserId", status: false})
}

if(!objectId(userId)){
  return res.send({msg: "enter valid user Id", status: false})
}

const savedData = await userModel.findById(userId)
